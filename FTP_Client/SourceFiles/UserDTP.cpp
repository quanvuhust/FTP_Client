...
bool UserDTP::download(char *path)
{
	HANDLE file = CreateFile(path, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_NEW, 0, NULL);
	int iResult = 1, sum = 0;
	char *recvbuf = new char[BLOCK_SIZE + 1];
	while (1) {
		iResult = dataSocket.receive(recvbuf, BLOCK_SIZE);
...
bool UserDTP::download(char *path, int startOffset, int size)
{
	HANDLE file = CreateFile(path, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_NEW, 0, NULL);
